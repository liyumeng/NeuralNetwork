...
void Train()
{
	...
	SampleLoader* loader = new SampleLoader("C:\\data\\medium\\norm\\train1217.bin");
	loader->LoadBin();
	...
	//loader->EnableTestLoader();
	//三层网络（1个隐藏层），每层的节点个数
	int unitCount[4] = { inputSize,1000,loader->OutputSize };
...
void Test()
{
	...
	//此后n行，为x及y的值，如1 0 1表示输入x={1,0},输出y={1}
	cout << "载入样本中..." << endl;
	SampleLoader* loader = new SampleLoader("C:\\data\\test1218.bin");
	loader->LoadBin();
	int inputSize = loader->InputSize;
	cout << "输入维度:" << inputSize << endl;

	//三层网络（1个隐藏层），每层的节点个数
	int unitCount[3] = { inputSize,1000,loader->OutputSize };
